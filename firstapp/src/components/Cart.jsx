...
export default function Cart() {
  ...
  async function fetchCartItems() {
    try {
      const res = await axios.get("https://ecommerce-i5cr.onrender.com/api/cart", {
        params: { userId },
      });
  ...
  async function removeItem(productId) {
    try {
      const res = await axios.post(
        "https://ecommerce-i5cr.onrender.com/api/cart/remove",
        { productId },
        { params: { userId } }
      );
      setCartItems(res.data.items);
  ...
  async function updateQuantity(productId, increment) {
    try {
      const res = await axios.post(
        "https://ecommerce-i5cr.onrender.com/api/cart/update-quantity",
        { productId, increment },
        { params: { userId } }
      );
      setCartItems(res.data.items);
